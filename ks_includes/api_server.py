"""Simple REST API server for triggering remote actions."""
...
class APIRequestHandler(BaseHTTPRequestHandler):
    ...
    def _send_json(self, data, status=200):
        """Send *data* as a JSON response with *status*."""
    ...
    def do_POST(self):  # noqa: N802 (method name is from BaseHTTPRequestHandler)
        """Trigger actions such as restart or reconnect."""
    ...
    def log_message(self, fmt, *args):
        """Route server log messages through the logging module."""
        logging.info("%s - %s", self.address_string(), fmt % args)


class APIServer(threading.Thread):
    """Background thread hosting the simple HTTP API server."""
    ...
    def run(self):
        ...
        try:
            ...
            logging.exception("API server stopped: %s", exc)
